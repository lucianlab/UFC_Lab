"""
UFC GRU Sequence Model v7 (True No-Leakage)
覆蓋: src/train_seq.py

配合 data_prep_seq_v7.py：
  - 從 matchup_samples.pkl 讀取，每筆 sample 直接帶 point-in-time snapshot
  - Dataset 不再查全域 sequence dict
  - 時間切割：train < 2024, val 2024-2025, cal 2024-2025（未對稱化）
...
cal_samples   = [s for s in all_samples if "2024-01-01" <= s["date"] < "2026-01-01"]
val_samples   = cal_samples  # val 用同一批，對稱化後評估

print(f"訓練集: {len(train_samples)} 筆（2024年以前）")
print(f"校準/驗證集: {len(cal_samples)} 筆（2024-2025）")

# ──────────────────────────────────────────────
# 3. 物理特徵（matchup features）
# ──────────────────────────────────────────────
...
val_ds       = FightSnapshotDataset(val_samples,   symmetrize=True)
cal_ds       = FightSnapshotDataset(cal_samples,   symmetrize=False)  # Platt: 不對稱化
...
cal.fit(raw_arr, true_arr)
print(f"校準前: {raw_arr.mean():.3f} → 校準後: {cal.predict_proba(raw_arr)[:,1].mean():.3f} "
...
with open("models/seq_fighter_id_map.json", "w") as f:
    json.dump({n: i for i, n in enumerate(fighter_names)}, f,
