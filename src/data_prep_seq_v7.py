"""
UFC Sequence Data Preparation v7 (True No-Leakage)
覆蓋: src/data_prep_seq.py

核心修正：
  每筆 matchup sample 直接帶著「該場比賽發生當下」的雙方序列快照
  訓練時不再用 fighter name 查全域 sequence dict
  → 真正的 point-in-time snapshot，完全沒有 look-ahead

輸出：
  data/processed/matchup_samples.pkl   ← 訓練用（帶 snapshot）
  data/processed/fighter_sequences.pkl ← predict/backtest 用（全量，不變）
  data/processed/seq_stats.json
...
print(f"序列特徵數: {N_FEATURES}, 序列長度: {SEQ_LEN}")

# ──────────────────────────────────────────────
# 9. 主迴圈：逐場建立選手序列 + point-in-time snapshot
# ──────────────────────────────────────────────
print("建立選手序列（含 point-in-time snapshot）...")

fighter_history = defaultdict(list)   # 累積歷史，用於最終全量 pkl
matchup_samples = []                   # 訓練用，每筆帶 snapshot

def make_vec(fighter, won, opp_name, event, bout,
             f_round, f_sec, f_type, t_rounds, div, tf):
    rounds = round_data.get((event, bout, fighter), {})
    if not rounds:
        return None
    ...
    total_sig_a  = sum(r.get("sig_a",0)    for r in rounds.values())
    ...
    opp_wr        = get_overall_wr(opp_name)   # 這場之前的對手勝率
    ...
    vec = [
        ...
        total_sig_l / duration_min,
        ctrl_R1, ctrl_R2, ctrl_trend,
        r1.get("dist_l",0)    / r1_struck,
        r_last.get("dist_l",0) / rl_struck,
        (r_last.get("sig_l",0) - r1.get("sig_l",0)) / (r1.get("sig_l",0) + 1e-6),
        float(f_round) / 5.0,
        opp_wr, float(tf), float(t_rounds) / 5.0,
...
def build_snapshot(history_list):
    """把 fighter_history 的最近 SEQ_LEN 筆轉成 (seq_array, mask_array)"""
...
for _, row in fr.iterrows():
    ...
    vec_a = make_vec(fighter_a, fighter_a==winner, fighter_b,
                     event, bout, f_round, f_sec, f_type, t_rounds, div, tf)
    vec_b = make_vec(fighter_b, fighter_b==winner, fighter_a,
                     event, bout, f_round, f_sec, f_type, t_rounds, div, tf)

    if vec_a is None or vec_b is None:
        # 沒有打擊數據，只更新勝率，不存 sample
    ...
    # ── 關鍵：在 append 之前 snapshot ──────────────
    # 此刻 fighter_history 只有「這場之前」的歷史
    if len(fighter_history[fighter_a]) >= 1 and \
       len(fighter_history[fighter_b]) >= 1:
        ...
        matchup_samples.append({
            ...
            "fighter_a":   fighter_a,
            "fighter_b":   fighter_b,
            "n_hist_a":    len(fighter_history[fighter_a]),  # snapshot 的有效場數
            ...
            "seq_a":   snap_a_seq,   # shape (SEQ_LEN, N_FEATURES)
            "mask_a":  snap_a_mask,  # shape (SEQ_LEN,)
            "seq_b":   snap_b_seq,
            "mask_b":  snap_b_mask,
        })
    ...
    # 更新累積勝率（在 append 之後，影響下一場的 opp_wr）
    for name, won in [(fighter_a, fighter_a==winner),
                      (fighter_b, fighter_b==winner)]:
        overall_fights[name] += 1
        div_fights[name][div] += 1
...
# ──────────────────────────────────────────────
# 10. 建立全量選手序列（給 predict/backtest 用）
# ──────────────────────────────────────────────
...
with open("data/processed/seq_stats.json", "w") as f:
    win_rate = sum(s["winner_is_a"] for s in matchup_samples) / len(matchup_samples)
    json.dump({
...
# 估算 pkl 大小
import os as _os
size_mb = _os.path.getsize("data/processed/matchup_samples.pkl") / 1e6
print(f"✅ 完成！")
print(f"  data/processed/matchup_samples.pkl  ({size_mb:.1f} MB, {len(matchup_samples)} samples)")
print(f"  data/processed/fighter_sequences.pkl ({len(fighter_seq_data)} 位選手)")

# ──────────────────────────────────────────────
# 13. 驗證：抽一筆確認 snapshot 時序正確
# ──────────────────────────────────────────────
print("\n驗證 snapshot 時序...")
for s in matchup_samples:
    if s["fighter_a"] == "Islam Makhachev" and s["n_hist_a"] >= 3:
        print(f"  Islam Makhachev 某場（{s['date']}）")
        print(f"  snapshot 有效場數: {s['n_hist_a']}（這場之前）")
        print(f"  mask: {s['mask_a']}")
        break
