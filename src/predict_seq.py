"""
UFC GRU Sequence Model — Prediction v5 (Production)
覆蓋: src/predict_seq.py

功能：
  - MC Dropout 信心區間（100次推論）
  - Platt Scaling 校準
  - 保守 Kelly（信心區間跨越50%時不押）
  - 市場賠率 Value 分析
  - 近期比賽顯示（dist%, td, kd, ctrl）
  - Attention 最重視的比賽
  - 相似選手（同量級向量空間）
...
def load_resources():
    ...
    return (seq_data, config, model, calibrator, embeddings,
            fighter_to_idx, matchups, fs, fr, all_names)
...
def get_reach_pct(name, division, seq_data):
    p    = seq_data.get("fighter_physical", {}).get(name, {})
    v    = p.get("reach")
    info = seq_data.get("div_physical", {}).get(division, {}).get("reach")
    if info is None or v is None: return 0.5
    sv = info.get("sorted", [])
    return sum(1 for x in sv if x < v) / len(sv) if sv else 0.5

def get_matchup_feat(fa, fb, title_fight, division, physical, config, seq_data):
...
# MC Dropout 預測
# ──────────────────────────────────────────────
def mc_predict(model, calibrator, a_seq, a_mask, b_seq, b_mask,
               mf_ab, mf_ba, n_samples=100):
    ...
    with torch.no_grad():
        for _ in range(n_samples):
            ...
            preds.append((p_ab + (1 - p_ba)) / 2)
    model.eval()

    arr      = np.array(preds)
    raw_mean = arr.mean()
    cal_mean = calibrator.predict_proba([[raw_mean]])[0][1] if calibrator else raw_mean

    return {
        "win_prob": cal_mean,
        "ci_low":   np.percentile(arr, 5),
        "ci_high":  np.percentile(arr, 95),
        "std":      arr.std(),
    }

def conservative_kelly(result, market_prob, dec_odds, max_kelly=0.20):
    ...
    conservative = ci_low if prob > 0.5 else 1 - ci_high
    edge = conservative - market_prob
    if edge <= 0:
        return 0.0, "保守估計無正 Edge"

    kelly = min(edge / (dec_odds - 1), max_kelly)
    return kelly, None
...
def recent_form(name, fs, fr, n=4):
    ...
    for _, row in fights.iterrows():
        ...
        fs_mask = (fs["EVENT"]==event) & (fs["BOUT"]==bout) & \
                  (fs["FIGHTER"].str.strip().str.lower()==name.lower())
        fs_rows = fs[fs_mask]
        ...
        results.append({
            "date": str(row["DATE"])[:10], "won": won, "opp": opp,
            "dist": dist/total*100,
            "td":   sum_col("TD"),
            "kd":   sum_col("KD"),
...
def find_similar(name, embeddings, fighter_to_idx, matchups, sequences, top_n=4):
    ...
    for i in np.argsort(-sims):
        if i == idx: continue
        fname = names[i]
        if get_main_division(fname, matchups) != own_div: continue
...
def predict_matchup(name_a, name_b, title_fight=False,
    ...
    (seq_data, config, model, calibrator, embeddings,
     fighter_to_idx, matchups, fs, fr, all_names) = load_resources()
    ...
    if info_a["n_fights"] < 5 or info_b["n_fights"] < 5:
        conf_label += "  ⚠️  場次不足，可信度低"
    ...
    if market_odds_a or market_odds_b:
        print(f"\n  Value 分析")
        mkt_a_raw = parse_american_odds(market_odds_a) if market_odds_a else None
        mkt_b_raw = parse_american_odds(market_odds_b) if market_odds_b else None

        if mkt_a_raw and mkt_b_raw:
            total = mkt_a_raw + mkt_b_raw
            mkt_a_clean = mkt_a_raw / total
            mkt_b_clean = mkt_b_raw / total
        else:
            mkt_a_clean = mkt_a_raw
            mkt_b_clean = mkt_b_raw

        if market_odds_a and mkt_a_clean:
            edge_a = win_prob_a - mkt_a_clean
            dec_a  = (1 + float(market_odds_a)/100) if float(market_odds_a) > 0 \
                     else (1 + 100/abs(float(market_odds_a)))
            kelly_a, reason_a = conservative_kelly(result_a, mkt_a_clean, dec_a)
            bet_a = (bankroll * kelly_a) if bankroll else None
            print(f"  {fa.split()[0]:<22} 市場:{market_odds_a:>6}  "
                  f"模型:{odds_a:>6}  Edge:{edge_a:+.1%}  {value_label(edge_a)}")
            print(f"  {'':22} 保守Kelly: {kelly_a*100:.1f}%"
                  + (f"  建議押注: ${bet_a:,.0f}" if bet_a else "")
                  + (f"  ({reason_a})" if reason_a else ""))

        if market_odds_b and mkt_b_clean:
            edge_b = win_prob_b - mkt_b_clean
            dec_b  = (1 + float(market_odds_b)/100) if float(market_odds_b) > 0 \
                     else (1 + 100/abs(float(market_odds_b)))
            result_b = {"win_prob": win_prob_b,
                        "ci_low": 1-ci_high_a, "ci_high": 1-ci_low_a}
            kelly_b, reason_b = conservative_kelly(result_b, mkt_b_clean, dec_b)
            bet_b = (bankroll * kelly_b) if bankroll else None
            print(f"  {fb.split()[0]:<22} 市場:{market_odds_b:>6}  "
                  f"模型:{odds_b:>6}  Edge:{edge_b:+.1%}  {value_label(edge_b)}")
            print(f"  {'':22} 保守Kelly: {kelly_b*100:.1f}%"
                  + (f"  建議押注: ${bet_b:,.0f}" if bet_b else "")
                  + (f"  ({reason_b})" if reason_b else ""))
...
if __name__ == "__main__":
    print("UFC GRU Sequence Predictor v5 (Production)")
    print("市場賠率格式：美式賠率，例如 -250 或 +180，不輸入直接 Enter")
    print("輸入 q 離開\n")
