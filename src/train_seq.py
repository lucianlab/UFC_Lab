"""
UFC GRU Sequence Model v5 (Production)
覆蓋: src/train_seq.py

架構：單頭 GRU + Attention（回到最成功的版本）
新增：Platt Scaling 校準器
...
print(f"校準集: {len(cal_orig)} 筆（2024-2025，A方勝率={cal_orig['winner_is_a'].mean():.2%}）")
...
class FightSeqDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (
            ...
            torch.tensor(float(row["winner_is_a"]), dtype=torch.float32),
        )
...
print(f"Predictor: {HIDDEN_DIM*2+MATCHUP_DIM} → 128 → 64 → 1（單頭）")
...
print(f"\n2026 Holdout val accuracy: {final_val_acc:.3f}")

# ──────────────────────────────────────────────
# 9. Platt Scaling（用未對稱化的 2024-2025）
# ──────────────────────────────────────────────
...
print(f"校準前: {raw_probs.mean():.3f} → 校準後: {cal_preds.mean():.3f} （實際: {true_labels.mean():.3f}）")
...
# 11. 2026 逐場回測
# ──────────────────────────────────────────────
print("2026 年回測請執行: python3 src/backtest_2026.py")
...
def find_similar(name, top_n=5):
    ...
    def get_div(n):
        m = mq[(mq["fighter_a"]==n)|(mq["fighter_b"]==n)]
...
print("\n驗證風格向量...")
for name in ["Carlos Prates","Islam Makhachev","Sean Strickland","Alex Pereira"]:
    find_similar(name)
