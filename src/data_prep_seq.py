"""
UFC Sequence Data Preparation v5 (Production)
...
序列特徵 28 個:
  ...
  對手近期 (2): opponent_wr_recent, opponent_wr_trend  ← 唯一新增
...
assert N_FEATURES == 28, f"特徵數應為28，實際為{N_FEATURES}"
...
for _, row in fr.iterrows():
    ...
    if len(fighter_history[fighter_a]) >= 1 and len(fighter_history[fighter_b]) >= 1:
        matchup_records.append({
            ...
            "fighter_a":     fighter_a,
            "fighter_b":     fighter_b,
            "a_seq_len":     len(fighter_history[fighter_a]),
            "b_seq_len":     len(fighter_history[fighter_b]),
